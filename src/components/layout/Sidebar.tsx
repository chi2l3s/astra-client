...
import { Home, Users, Package, Settings, Gamepad2, Shirt, PanelLeft, PanelRight } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ collapsed, onToggle }) => {
  ...
  return (
    ...
    >
      <div className={cn('mb-8 relative flex items-center justify-between', collapsed ? 'w-full px-0' : 'w-full pr-3')}>
        ...
        <button
          onClick={onToggle}
          className={cn(
            'absolute top-1/2 -translate-y-1/2 w-9 h-9 rounded-xl bg-white/5 border border-white/10 hover:border-primary/50 hover:bg-white/10 transition-all duration-300 flex items-center justify-center',
            collapsed
      ...
      <nav className={cn('flex-1 flex flex-col gap-3 w-full', collapsed ? 'px-3' : '')}>
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            className={({ isActive }) =>
              cn(
                ...
                collapsed ? 'justify-center w-full aspect-square' : 'justify-start w-full px-4 py-3 gap-3',
                isActive
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon
                  className={cn('w-6 h-6 transition-transform duration-300', !isActive && 'group-hover:scale-110')}
                />
      ...
      <div className={cn('mt-auto', collapsed ? 'px-3' : '')}>
        ...
        >
          <div className={cn('rounded-xl overflow-hidden', collapsed ? 'w-12 h-12' : 'w-10 h-10')}>
            {activeAccount ? (
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium truncate">{activeAccount?.username || 'Войти'}</div>
