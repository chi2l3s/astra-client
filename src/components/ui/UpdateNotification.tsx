...
export const UpdateNotification = () => {
  ...
  useEffect(() => {
    if (window.astra) {
      const removeListener = window.astra.updates.onStatus((data: UpdateStatus) => {
      ...
      >
        ...
        {status.status === 'progress' && (
          <div className="space-y-3">
            ...
            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-primary transition-all duration-300" style={{ width: `${status.progress || 0}%` }} />
            </div>
          </div>
        )}
        ...
        {status.status === 'downloaded' && (
          <div className="space-y-3">
            ...
            <Button size="sm" onClick={handleRestart} className="w-full" leftIcon={<Download className="w-4 h-4" />}>
              Перезапустить
            </Button>
          </div>
        )}
