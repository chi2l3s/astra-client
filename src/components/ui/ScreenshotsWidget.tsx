...
export const ScreenshotsWidget = () => {
  ...
  const fetchScreenshots = async () => {
    if (!selectedVersion || !window.electron) return;
    
    setLoading(true);
    try {
      const data = await window.electron.ipcRenderer.invoke('get-screenshots', { versionId: selectedVersion });
      setScreenshots(data);
    } catch (e) {
  ...
  return (
    <div className="bg-black/20 rounded-3xl p-6 border border-white/5 flex flex-col gap-4 min-h-[300px]">
       ...
       {loading && screenshots.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-text-secondary/50">
             Загрузка...
          </div>
